...
void DrawBox(BYTE *Raw_Intensity, unsigned int Height, unsigned int Width) {

	int karekenar = 80;
	int y = (Width / 2) - karekenar / 2;

	for (int i = Width * y + y; i <= Width * y + y + karekenar; i++) {
		Raw_Intensity[i] = 0;//üst çizgi
		Raw_Intensity[i + Width * karekenar] = 0;//alt cizgi
	}
	
	for (int i = Width * y + y; i < Width*(y + karekenar) + y + karekenar; i += Width) {
		Raw_Intensity[i] = 0;  //sol cizgi
		Raw_Intensity[i + karekenar] = 0;// sag cizgi 
	}
}
